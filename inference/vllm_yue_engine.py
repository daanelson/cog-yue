...
class BlockTokenRangeProcessor(LogitsProcessor):
    """Filter decoding to only audio tokens"""
...
class VLLMYue:
    def __init__(self, device="cuda"):
        ...
        state_dict = torch.load(codec_path, map_location='cpu', weights_only=False)
    ...
    def main_generate(self, genre_input: str, lyrics_input: str, max_new_tokens: int, run_n_segments: int, seed: int, output_dir: str):
        ...
        timer.time(f"Created output directories: {stage1_output_dir}, {stage2_output_dir}")
        ...
        def load_audio_mono(filepath, sampling_rate=16000):
            timer.time(f"Loading audio file: {filepath}")
            audio, sr = torchaudio.load(filepath)
            # Convert to mono
            audio = torch.mean(audio, dim=0, keepdim=True)
            # Resample if needed
            if sr != sampling_rate:
                timer.time(f"Resampling audio from {sr}Hz to {sampling_rate}Hz")
                resampler = Resample(orig_freq=sr, new_freq=sampling_rate)
                audio = resampler(audio)
            return audio

        def encode_audio(codec_model, audio_prompt, device, target_bw=0.5):
            timer.time("Encoding audio prompt...")
        ...
        def split_lyrics(lyrics):
            timer.time("Splitting lyrics into segments...")
            pattern = r"\[(\w+)\](.*?)\n(?=\[|\Z)"
            segments = re.findall(pattern, lyrics, re.DOTALL)
            structured_lyrics = [f"[{seg[0]}]\n{seg[1].strip()}\n\n" for seg in segments]
            timer.time(f"Found {len(structured_lyrics)} lyric segments")
        ...
        timer.time("Loading genre tags and lyrics...")
        ...
        timer.time(f"Genre tags: {genres}")
        timer.time(f"Number of lyric segments: {len(lyrics)}")
        ...
        for i, p in enumerate(tqdm(prompt_texts[:run_n_segments], desc="Stage1 inference...")):
            timer.time(f"\nProcessing segment {i}/{run_n_segments-1}")
            section_text = p.replace('[start_of_segment]', '').replace('[end_of_segment]', '')
            if i==0:
                continue
            if i==1:
                if args.use_dual_tracks_prompt or args.use_audio_prompt:
                    timer.time("Processing audio prompt...")
                    if args.use_dual_tracks_prompt:
                        timer.time("Using dual tracks prompt")
                        ...
                        vocals_ids = encode_audio(self.codec_model, vocals_ids, device, target_bw=0.5)
                        instrumental_ids = encode_audio(self.codec_model, instrumental_ids, device, target_bw=0.5)
                        ...
                        timer.time("Using single audio prompt")
                        audio_prompt = load_audio_mono(args.audio_prompt_path)
                        raw_codes = encode_audio(self.codec_model, audio_prompt, device, target_bw=0.5)
                        # Format audio prompt
                    ...
                    audio_prompt_codec_ids = [mmtokenizer.soa] + self.codectool.sep_ids + audio_prompt_codec + [mmtokenizer.eoa]
                    sentence_ids = mmtokenizer.tokenize("[start_of_reference]") +  audio_prompt_codec_ids + mmtokenizer.tokenize("[end_of_reference]")
                    head_id = mmtokenizer.tokenize(prompt_texts[0]) + sentence_ids
                else:
                    head_id = mmtokenizer.tokenize(prompt_texts[0])
                prompt_ids = head_id + start_of_segment + mmtokenizer.tokenize(section_text) + [mmtokenizer.soa] + self.codectool.sep_ids
            else:
                prompt_ids = end_of_segment + start_of_segment + mmtokenizer.tokenize(section_text) + [mmtokenizer.soa] + self.codectool.sep_ids
            ...
            with torch.no_grad():
                ...
                output_seq = stage1_model.generate(prompt, sampling_params)
        ...
        for i in range(range_begin, len(soa_idx)):
            timer.time(f"Processing segment {i} of {len(soa_idx)}")
        ...
        timer.time(f"Saving Stage 1 outputs to:\n{vocal_save_path}\n{inst_save_path}")
        ...
        stage2_result = self.stage2_inference(stage1_output_set, stage2_output_dir, batch_size=args.stage2_batch_size)
        timer.time(f"Stage 2 outputs: {stage2_result}")
        timer.time('Stage 2 DONE.\n')
        ...
        for npy in stage2_result:
            timer.time(f"Processing {npy}")
            codec_result = np.load(npy)
            decodec_rlt=[]
            with torch.no_grad():
                decoded_waveform = self.codec_model.decode(torch.as_tensor(codec_result.astype(np.int16), dtype=torch.long).unsqueeze(0).permute(1, 0, 2).to(device))
            decoded_waveform = decoded_waveform.cpu().squeeze(0)
            decodec_rlt.append(torch.as_tensor(decoded_waveform))
            decodec_rlt = torch.cat(decodec_rlt, dim=-1)
            save_path = os.path.join(recons_output_dir, os.path.splitext(os.path.basename(npy))[0] + ".mp3")
            tracks.append(save_path)
            timer.time(f"Saving reconstructed audio to {save_path}")
            save_audio(decodec_rlt, save_path, 16000)
        # mix tracks
        timer.time("Mixing tracks...")
        for inst_path in tracks:
            try:
                if (inst_path.endswith('.wav') or inst_path.endswith('.mp3')) \
                    ...
                    timer.time(f"Creating mix: {recons_mix}")
                    vocal_stem, sr = sf.read(inst_path)
                    instrumental_stem, _ = sf.read(vocal_path)
                    mix_stem = (vocal_stem + instrumental_stem) / 1
                    sf.write(recons_mix, mix_stem, sr)
            except Exception as e:
                timer.time(f"Error mixing tracks: {e}")
        ...
        try:
            ...
            timer.time(f"Created mix: {vocoder_mix}")
        except RuntimeError as e:
            timer.time(e)
            timer.time(f"mix {vocoder_mix} failed! inst: {instrumental_output.shape}, vocal: {vocal_output.shape}")
